    ...
    ({ budget_id: budgetId, month }) => {
      ...
      return promptResult(
        ...
        [
          ...
          "",
          "1) Call `get_budget_health` for overall status — net worth, overspending, target progress, RTA, and credit card gaps.",
          "2) If mid-month, call `get_monthly_budget` and compare each category's activity against its budgeted amount and the day of the month to judge spending pace. Account for lumpy categories (rent, insurance) that spend all at once.",
          "3) Call `get_scheduled_transactions` for the rest of the month to see committed outflows per category.",
          "4) If overspending is found, propose budget moves yourself: pull from categories with surplus balances, but check `get_targets` first so you never drain an underfunded target or savings goal without flagging it.",
          "5) If RTA > 0, propose an allocation yourself using `get_targets` (underfunded amounts and deadlines) and recent spending from `get_spending_analysis` — explain your prioritization.",
          "6) Call `detect_anomalies` to flag unusual transactions that may need review.",
          "7) Suggest concrete actions (reassign budget via `set_category_budgets`, recategorize, adjust scheduled transactions) and report `undo_history_ids` after any writes.",
        ].join("\n"),
      );
    },
  );
    ...
    ({ budget_id: budgetId }) =>
      promptResult(
        ...
        [
          ...
          "- `ynab://knowledge/credit-cards` — Credit card payment mechanics",
          "",
          "1) Call `get_budget_health` for current budget state — overspending, target gaps, RTA, credit card payment gaps.",
          ...
          "4) Model a better allocation yourself: combine `get_targets` (underfunded amounts, deadlines) with average spending from `get_spending_analysis`, and propose specific `set_category_budgets` changes with your reasoning.",
          "5) If `get_budget_health` reports credit card payment gaps, use the `ynab://knowledge/credit-cards` mechanics to explain the debt and propose extra payment-category funding on top of what is already budgeted.",
